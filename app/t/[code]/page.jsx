...
export default async function TagPage({ params }) {
  ...
  // Only select non-identifying fields — this page is public and must never
  // reveal the owner's name, phone, email, address, or notes.
  const tag = await prisma.tag.findUnique({
    where: { code: code.toUpperCase() },
    select: { code: true, vehicleReg: true, vehicleMakeModel: true },
  });
    ...
    <div className="flex min-h-full flex-col items-center justify-center bg-gradient-to-b from-yellow-50 to-slate-50 px-6 py-16 dark:from-yellow-500/10 dark:to-slate-950">
      <div className="flex w-full max-w-md flex-col items-center gap-6 text-center">
        ...
        <p className="max-w-sm text-sm text-slate-500 dark:text-slate-400">
          The owner&apos;s name and phone number are never shown here. Send a message below and
          it goes straight to them.
        </p>

        <ContactOwnerForm code={tag.code} />
