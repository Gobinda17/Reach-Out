...
**Emergency routing**: `User.emergencyPhone` (settable from `/dashboard`'s Profile section, via `EmergencyPhoneForm.jsx` → `POST /api/account/emergency-phone`) is a plain profile field, not OTP-verified — it's never used to authenticate, only as an alternate ring target. `Call.targetPhone` is resolved once, at allocation time, in `POST /api/tags/[code]/call`: the tag owner's `emergencyPhone` when `reason` is `"emergency"` (`isEmergencyReason()`, `lib/contactReasons.js`) *and* one is set, otherwise the tag's own `phone` — same as every other reason. The edesy webhook (`app/api/webhooks/edesy/route.js`) matches against `Call.targetPhone` rather than re-deriving it from `tag.phone`, so a live emergency call bridges to the emergency number; `tag.phone` is kept there only as a fallback for `Call` rows created before this column existed.
