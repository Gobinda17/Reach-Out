...
`lib/calling.js` exports `CALL_PROVIDERS` (currently `dev` and `edesy`) and `allocateVirtualNumber({ ownerPhone, callerPhone, reference })`. Every real provider needs the caller's phone number up front — there's no way to hand an anonymous scanner a dial-able masked number without telling the provider who they are — so `CallOwnerPanel.jsx` collects it before requesting a call. Adding a new provider means adding a case to `allocateVirtualNumber()` and a new entry in `CALL_PROVIDERS`; an admin can then switch to it from `/admin/settings` without a redeploy, but the integration code itself still has to be written — the settings page only switches between providers that already exist in code.
