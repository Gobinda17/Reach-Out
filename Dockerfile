...
# Node 24, not 22, on purpose. package-lock.json was generated by npm 11
# (bundled with Node 24); node:22-alpine ships npm 10, whose optional-dependency
# resolution disagrees with it, and `npm ci` then aborts with
#   "Missing: @emnapi/runtime from lock file".
# The lockfile is not corrupt — it just has to be read by the npm major that
# wrote it. Regenerate it with the same npm as this base image if you change it.
