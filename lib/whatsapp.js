...
// and the owner's name + vehicle reg are the only two variables (the typed
// message itself isn't sent here — it's already saved and visible on the
// owner's dashboard, this is just a heads-up). Best-effort: a failure here
// must never fail the scan-message request.
export async function sendScanNotification({ to, reasonKey, ownerName, vehicleReg }) {
