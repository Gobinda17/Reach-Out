...
// and owner name / vehicle reg / note are its three variables. `note` must
// always be a non-empty string — WhatsApp templates can't conditionally hide
// a variable's line, so the caller is responsible for a fallback when
// nothing was typed. The message itself is still saved and shown in full on
// the owner's dashboard regardless of what fits here. Best-effort: a failure
// here must never fail the scan-message request.
export async function sendScanNotification({ to, reasonKey, ownerName, vehicleReg, note }) {
...
async function otpTemplateName() {
  return (
    (await getSetting("WHATSAPP_OTP_TEMPLATE", process.env.WHATSAPP_OTP_TEMPLATE)) ||
    "reachout_qr_otp"
  );
}

async function otpTemplateLang() {
  return (await getSetting("WHATSAPP_OTP_TEMPLATE_LANG", process.env.WHATSAPP_OTP_TEMPLATE_LANG)) || "en";
}

// Delivers a login OTP over WhatsApp using an Authentication-category
// template. Authentication templates have Meta-fixed body wording (no custom
// copy like sendScanNotification's reason templates) and a "Copy code" button
// component — not zero-tap/one-tap autofill, which need a linked native
// Android app and don't apply to a website login form. Falls back to the
// caller logging the code server-side (see lib/sms.js) whenever this returns
// false, so a WhatsApp outage never blocks someone signing in.
export async function sendWhatsAppOtp(phone, code) {
  if (!(await whatsappNotifyConfigured())) return false;

  const token = await accessToken();
  const phoneId = await phoneNumberId();
  const language = await otpTemplateLang();
  const template = await otpTemplateName();
    ...
    body: JSON.stringify({
      ...
      template: {
        ...
        components: [
          ...
          {
            // Meta's Cloud API represents every authentication-template button
            // — including "Copy code", which is what this template actually
            // uses — as sub_type "url" in the send payload; "copy_code" (a
            // template-management-API concept) is rejected here with
  ...
  if (!res.ok) {
    const body = await res.text().catch(() => "");
    console.error(`[whatsapp] OTP send failed for template "${template}" (${res.status}): ${body}`);
    return false;
  }
