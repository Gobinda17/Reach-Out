...
// `new Error(x)` silently stringifies a non-string x to "[object Object]" —
// edesy's error body isn't guaranteed to be a flat string (could be nested,
// e.g. `{ error: { message, code } }`), so this pulls out whatever string is
// actually in there instead of losing it. Falls back to the raw JSON, still
// more useful to whoever reads the 502 than "[object Object]" was.
function extractEdesyErrorMessage(data, status) {
  if (!data) return `Masking provider request failed (${status}).`;

  const candidate = data.message ?? data.error ?? data.error_description ?? data.detail;
  if (typeof candidate === "string" && candidate) return candidate;
  if (candidate && typeof candidate === "object") {
    const nested = candidate.message ?? candidate.error ?? candidate.description;
  ...
  try {
    ...
    return `Masking provider request failed (${status}).`;
  }
}
...
async function allocateEdesySession({ ownerPhone, callerPhone, reference }) {
  ...
  if (!res.ok) {
    // console.error, not warn: this is the only place the real edesy error
    // body is visible — extractEdesyErrorMessage below deliberately falls
    // back to a generic string when the shape is unrecognized, so the raw
    // JSON here is what tells us what to adjust once we see real traffic.
    console.error("[edesy] session request failed:", res.status, JSON.stringify(data));
    throw new CallProviderError(extractEdesyErrorMessage(data, res.status));
  }
